...
class Favorite extends Controller {
  ...
  async create(ctx, next) {
    const user = await this.findByField('userid', ctx.request.body.title);

    if (user) {
      ctx.status = 403;
      ctx.body = { error: 'Not unique user' };
    } else {
  ...
  async getByUserId(ctx) {
    const { id } = ctx.params;
    const item = await this.findByField('userid', id);
...
module.exports = new Favorite('favorites');
